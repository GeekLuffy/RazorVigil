...
| **Adversarial-Realistic Recall** | **97.60%** `[96.20%, 98.80%]` | **97.00%** `[95.60%, 98.40%]` | **97.00%** `[95.60%, 98.40%]` | Stealth human-mimicking bot segment (n=500) |
...
|---|---|---|---|---|
| $\tau_{\text{if}}$ | Isolation Forest threshold | `[0.45, 0.50, 0.55]` | **0.45** | Validation partition only (D_val, N=10,000) |
| $\tau_{\text{sup}}$ | Supervised risk ceiling | `[0.30, 0.35, 0.40]` | **0.40** | Validation partition only (D_val, N=10,000) |
| $\theta_{\text{cvv}}$ | CVV cycle attempt cutoff | `[2.0, 3.0, 4.0]` | **3.0** | Validation partition only (D_val, N=10,000) |
| $\theta_{\text{entropy}}$ | Keystroke entropy ceiling | `[0.60, 0.80, 1.00]` | **0.60** | Validation partition only (D_val, N=10,000) |
| $\theta_{\text{time}}$ | Time on page floor | `[1.5s, 2.5s, 3.5s]` | **1.5s** | Validation partition only (D_val, N=10,000) |
| $\theta_{\text{bin}}$ | Device distinct BIN cutoff | `[2.0, 3.0, 4.0]` | **4.0** | Validation partition only (D_val, N=10,000) |
| $\theta_{\text{fanout}}$ | Rotating IP & PAN pair | `[(4,4), (6,6), (8,8)]` | **(8.0, 8.0)** | Validation partition only (D_val, N=10,000) |
...
- **Documented Selection Rule**: Maximize Zero-Day CVV Recall subject to Validation Edge-Case Genuine FPR <= 10.0%. Operating Point **P2** was selected.
<!-- METRICS_GATE:END -->
...
| **Slow Distributed Carding** | 1,000 | 100.0% | Recall: **100.00%** `[100.00%, 100.00%]` | Recall: **100.00%** `[100.00%, 100.00%]` | Recall: **100.00%** `[100.00%, 100.00%]` |
| **Rapid Burst Script Botnets** | 1,000 | 100.0% | Recall: **100.00%** `[100.00%, 100.00%]` | Recall: **100.00%** `[100.00%, 100.00%]` | Recall: **100.00%** `[100.00%, 100.00%]` |
| **Adversarial Realistic Bots** | 500 | 100.0% | Recall: **97.60%** `[96.20%, 98.80%]` | Recall: **97.00%** `[95.60%, 98.40%]` | Recall: **97.00%** `[95.60%, 98.40%]` |
| **CVV Cycling (In-Domain)** | 500 | 100.0% | Recall: **100.00%** `[100.00%, 100.00%]` | Recall: **100.00%** `[100.00%, 100.00%]` | Recall: **100.00%** `[100.00%, 100.00%]` |
<!-- METRICS_SEGMENTS:END -->

---

### 📐 Closed-Form Mathematical Proof: Global ROC-AUC Derivation

<!-- METRICS_WILCOXON:START -->
...
1. **Clean Positives vs. Clean Negatives** ($w_1 = \frac{16250000}{21000000} = 77.3810%$): $\text{AUC}_1 = \mathbf{1.000000}$
2. **Clean Positives vs. Hard Negatives** ($w_2 = \frac{1250000}{21000000} = 5.9524%$): $\text{AUC}_2 = \mathbf{0.999761}$
3. **Ambiguous Positives vs. Clean Negatives** ($w_3 = \frac{3250000}{21000000} = 15.4762%$): $\text{AUC}_3 = \mathbf{0.999909}$
4. **Ambiguous Positives vs. Hard Negatives** ($w_4 = \frac{250000}{21000000} = 1.1905%$): $\text{AUC}_4 = \mathbf{0.990968}$
