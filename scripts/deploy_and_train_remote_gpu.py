...
LOCAL_CSV = "backend/dataset/synthetic_transactions_50k.csv"
REMOTE_DIR = "/home/big/razorshield_gpu"

def run_remote_python(code_str, timeout=120):
    ...
    try:
        ws = websocket.create_connection(f"{BASE_WS}/api/kernels/{kernel_id}/channels?token={TOKEN}", timeout=15)
        msg_id = str(uuid.uuid4())
        exec_msg = {
            "header": {"msg_id": msg_id, "username": "big", "session": str(uuid.uuid4()), "msg_type": "execute_request", "version": "5.3"},
            "metadata": {},
            "content": {"code": code_str, "silent": False, "store_history": True, "user_expressions": {}, "allow_stdin": False},
            "buffers": [],
            "parent_header": {},
            "channel": "shell"
        }
        ...
        while time.time() < t_end:
            ...
            if m_type == "stream":
                ...
                err = msg.get("content", {}).get("evalue", "")
                print("ERROR:", err)
                print("\n".join(msg.get("content", {}).get("traceback", [])))
                output_chunks.append(f"ERROR: {err}")
...
print("[1/3] Reading local CSV dataset...")
with open(LOCAL_CSV, "rb") as f:
    csv_bytes = f.read()
b64_data = base64.b64encode(csv_bytes).decode("ascii")

print(f"[2/3] Uploading dataset to bd216server3 ({len(csv_bytes)/(1024*1024):.2f} MB)...")
upload_code = f"""
import os, base64

os.makedirs('{REMOTE_DIR}', exist_ok=True)
csv_path = os.path.join('{REMOTE_DIR}', 'synthetic_transactions_50k.csv')
raw_b64 = '''{b64_data}'''
with open(csv_path, 'wb') as f:
    f.write(base64.b64decode(raw_b64))

print("Uploaded CSV to:", csv_path, "Size (bytes):", os.path.getsize(csv_path))
"""
run_remote_python(upload_code)

print("[3/3] Launching GPU Training Job on bd216server3 (CUDA:4)...")
training_script = """
...
cb_val_preds = cb.predict_proba(X_val)[:, 1]
...
run_remote_python(training_script, timeout=180)
