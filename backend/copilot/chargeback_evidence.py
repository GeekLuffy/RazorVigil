"""
RazorVigil Sentinel — AI Chargeback Defense & Evidence Dossier Synthesizer.
...
Automated, zero-hallucination dispute evidence compilation grounded in:
  ...
  5. Regulatory & CoFT Compliance (Reserve Bank of India (Authentication Mechanisms for Digital Payment Transactions) Directions, 2025 (effective April 1, 2026), CoFT)
...
INITIAL_DISPUTE_CASES: List[Dict[str, Any]] = [
    ...
    },
    {
        ...
        "customer_email": "canary_trap_07@razorvigil.internal",
        "merchant_name": "SneakerVault India",
        "status": "PENDING_REVIEW",
...
class ChargebackEvidenceSynthesizer:
    ...
    def synthesize_evidence(self, case_id: str) -> Optional[EvidencePackage]:
        ...
        claims.append(VerifiableClaim(
            ...
            claim_title="RBI Authentication Directions & CoFT Tokenization Audit",
            factual_content=f"Transaction authenticated and processed in compliance with the Reserve Bank of India (Authentication Mechanisms for Digital Payment Transactions) Directions, 2025 (effective April 1, 2026). Card-on-File Tokenization (CoFT) active. {'Cryptographic AP2 Agent Mandate Verified.' if is_agent else 'Risk-based dynamic AFA 2FA authentication factor verification recorded.'}",
        ...
        representation_letter = (
            ...
            f"[This document is a draft prepared by RazorVigil Sentinel for human review. "
            ...
            f"5. REGULATORY CONTEXT: Authenticated in accordance with the Reserve Bank of India (Authentication Mechanisms "
            f"for Digital Payment Transactions) Directions, 2025 (effective April 1, 2026) and CoFT tokenization mandate.\n\n"
            f"RECOMMENDED NEXT STEP: {action_text}\n\n"
            f"Generated by: RazorVigil Sentinel AI Evidence Dossier Generator v1.2 — DRAFT ONLY"
        )

        rbi_attestation = (
            f"RBI-DRAFT-{case.case_id}: Evidence collected under framework aligned with the Reserve Bank of India "
            f"(Authentication Mechanisms for Digital Payment Transactions) Directions, 2025 (effective April 1, 2026). "
